...
public class StartDeeplinkJava extends CustomJavaAction<Boolean>
{
	...
	public Boolean executeAction() throws Exception
	{
		...
		Core.addRequestHandler(deeplink.proxies.constants.Constants.getRequestHandlerName() + "/",
				new DeepLinkHandler());
		if (deeplink.proxies.constants.Constants.getEnableSEO()) {
			Core.addRequestHandler("sitemap.xml", new SitemapRequestHandler());
		}
		return true;
		// END USER CODE
	}
	...
	class DeepLinkHandler extends RequestHandler {
		...
		private DeeplinkSEO dlseo = new DeeplinkSEO("main_template.fthl");
		...
		public void processRequest(IMxRuntimeRequest request, IMxRuntimeResponse response, String arg2)
			...
			try {
				...
				if (request.getParameter(ARGUSER) != null && request.getParameter(ARGPASS) != null) {
					...
					if (DeeplinkSEO.isBotRequest(getContext(), request.getHeader("User-Agent"))) {
						
						IContext ctx = Core.createSystemContext();
						IMendixObject deeplinkObj = query(ctx, DeepLink.getType(), DeepLink.MemberNames.Name, args[2]);
						
						if (deeplinkObj == null) {
							StartDeeplinkJava.logger.warn("Deeplink with name '" + args[2] + "' for SEO not found. ");
							serve404(request, response, session == null ? null : session.getUser().getName());
							return;
						}

						DeepLink deeplink = DeepLink.initialize(ctx, deeplinkObj);
						
						if (deeplink.getSupportsSEO() == false) {
							StartDeeplinkJava.logger.warn("This deeplink does not support SEO requests. ");
							serve404(request, response, session == null ? null : session.getUser().getName());
							return;
						}
						
						SeoEntityMetadata seoEntityMetadata = deeplink.getDeepLink_SeoEntityMetadata();
						
						if (seoEntityMetadata == null) {
							StartDeeplinkJava.logger.warn("SEO template for deeplink with name '" + args[2] + "' not found. ");
							serve404(request, response, session == null ? null : session.getUser().getName());
							return;
						}
						...
						if (!deeplink.getUseStringArgument() && deeplink.getObjectType() != null
								...
								&& theobject == null) {
							// Object expected but not found, getDeeplinkObject should already serve a 404.
							return;
						}
						
						String pageStr = dlseo.renderPage(seoEntityMetadata, theobject);
		...
		private void serveDeeplink(String[] args, IMxRuntimeRequest request, IMxRuntimeResponse response,
		...
		private IMendixObject getDeeplinkObject(String[] args, IMxRuntimeRequest request, IMxRuntimeResponse response,
			...
			if (!deeplink.getUseStringArgument() && deeplink.getObjectType() != null
				...
				if (deeplink.getObjectAttribute() == null || deeplink.getObjectAttribute().isEmpty())
					...
					arg = query(context, deeplink.getObjectType(), deeplink.getObjectAttribute(), argument);
	...
	class SitemapRequestHandler extends RequestHandler {
		private DeeplinkSEO dlseo = new DeeplinkSEO("sitemap.fthl");

		@Override
		public void processRequest(IMxRuntimeRequest request, IMxRuntimeResponse response, String arg2)
				throws Exception {
			IContext ctx = getContext();
			dlseo.serveSitemap(ctx, request, response);
		}
	}
